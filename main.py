...
from agent_manager import AgentManager, ClearMemory, MemoryInput, MemoryOutput
...
from functions_manager import FunctionInput, FunctionOutput, FunctionsManager
from preferences_resolver import QueryPreferencesInput
from queryplan_manager import QueryPlanInput, QueryPlanManager
from rate_limiter import RateLimiter, SyncRateLimiter
from web_manager import CacheHTML, HTMLInput, WebManager
...
async def getPreferences(preferences_query: QueryPreferencesInput):
    ...
    start = time.time()
    response = await agent_manager.preferences_resolver.get_preferences(
        preferences_query.user_id
    )
    if response is None:
        logging.info(
            f"Preferences for user {preferences_query.user_id} does not exist, returnng default and making one in the background..."
        )
        asyncio.create_task(
            agent_manager.preferences_resolver.create_default_preferences(
                preferences_query.user_id
            )
        )
...
async def writeQueryPlan(query_input: QueryPlanInput):
    ...
    logging.info(
        f"Writing query plan for conversation_id {query_input.conversation_id}"
    )
    response, elapsed_time = await queryplan_manager.query_plan(
        agent_manager.preferences_resolver, query_input
    )
...
async def writeMemoryForUser(memory_output: MemoryOutput):
    ...
    logging.info(
        f"Writing memory for user for user {memory_output.user_id}, conversation {memory_output.conversation_id}"
    )
...
async def pushFunctions(function_output: FunctionOutput):
    ...
    for function_item in function_output.functions:
        ...
        if function_item.category not in function_types:
            return {
                "response": f"Invalid category for function {function_item.name}, must be one of {function_types}"
            }
    ...
    # Push the functions
    result, elapsed_time = await functions_manager.push_functions(
        function_output.user_id, function_output.api_key, functions
    )
    return {"response": result, "elapsed_time": elapsed_time}
