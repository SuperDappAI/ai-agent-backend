...
async def test_flexible_document_summarizer():
    ...
    mock_llm.agenerate.return_value = MagicMock(
        generations=[
            [MagicMock(text="user summary text")],
            [MagicMock(text="aida summary text")],
        ]
    )
    ...
    mock_document.page_content = (
        '{"user": "user original text", "AiDA": "aida original text"}'
    )
    ...
    expected_prompt = SummaryPrompt(
        summarizations=2, importance="high"
    ).to_prompt_string()
    expected_messages = [
        [
            SystemMessage(content=expected_prompt),
            HumanMessage(content="user original text"),
        ],
        [
            SystemMessage(content=expected_prompt),
            HumanMessage(content="aida original text"),
        ],
    ]
    ...
    assert (
        mock_document.page_content
        == '{"user": "user summary text", "AiDA": "aida summary text"}'
    )
