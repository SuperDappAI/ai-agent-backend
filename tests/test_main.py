...
def test_query_plan():
    response = client.post("/query_plan/", data=dict(query="Compare the temperature in Sydney to that of London today"))
...
def test_semantic_search_html():
    ...
    response = client.post("/semantic_search_html/", json=mock_data)
    assert response.status_code == 200
    assert "response" in response.json()
    assert "elapsed_time" in response.json()
...
def test_get_functions():
    ...
    response = client.post("/get_functions/", json=mock_data)
    assert response.status_code == 200
    assert "response" in response.json()
    assert "elapsed_time" in response.json()
    print(response.json())
...
def test_clear_user_memory():
    ...
    response = client.post("/clear_conversation/", json=mock_data)
    assert response.status_code == 200
    assert "response" in response.json()
    assert "elapsed_time" in response.json()
