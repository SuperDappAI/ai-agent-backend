...
async def test_push_memory(setup_agent_manager):
    ...
    with patch.object(
        agent_manager,
        "load",
        return_value=GenerativeAgentMemory(
...
def test_create_new_memory_retriever(setup_agent_manager):
    ...
    with (
        patch.object(
            agent_manager.client, "create_collection"
        ) as mock_create_collection,
        patch.object(
            agent_manager.client, "create_payload_index"
        ) as mock_create_payload_index,
    ):
        result = agent_manager.create_new_memory_retriever(
            "test_api_key", "test_user_id"
        )
