class ClassifyPrompts:
    def __init__(self):
        CoTClassifyPrompt = {
            "Rationale": "This type is for complex queries that require a step-by-step logical reasoning process. It's ideal for solving puzzles, mathematical problems, or any query that demands a rigorous logical approach.",
        ...
        self.CoTClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in CoTClassifyPrompt.items()
        )
        ...
        CodeClassifyPrompt = {
            "Rationale": "This type is for queries related to coding, algorithms, or technical issues. While it may involve logical reasoning, the focus is more on the technical aspects.",
        ...
        self.CodeClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in CodeClassifyPrompt.items()
        )
        ...
        QAClassifyPrompt = {
            "Rationale": "This type is for straightforward questions that require a simple answer without the need for extensive reasoning or elaboration.",
        ...
        self.QAClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in QAClassifyPrompt.items()
        )
        ...
        ConversationClassifyPrompt = {
            "Rationale": "This is the default type that is for queries that are more conversational in nature and do not require a specific format or structure.",
        ...
        self.ConversationClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in ConversationClassifyPrompt.items()
        )

        EmotionClassifyPrompt = {
            "Rationale": "This type is for queries that seek emotional support, advice, or guidance on personal matters.",
        ...
        self.EmotionClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in EmotionClassifyPrompt.items()
        )
        ...
        CreativeClassifyPrompt = {
            "Rationale": "This type is for queries that ask for creative output, like writing a poem, story, or generating art.",
        ...
        self.CreativeClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in CreativeClassifyPrompt.items()
        )
        ...
        EducationalClassifyPrompt = {
            "Rationale": "This type is for queries that seek educational information or a tutorial on how to do something.",
        ...
        self.EducationalClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in EducationalClassifyPrompt.items()
        )
        ...
        FactualClassifyPrompt = {
            "Rationale": "This type is for queries that require a detailed, factual answer based on research or data.",
        ...
        self.FactualClassifyPrompt = "\n".join(
            f"{key}: {value}" for key, value in FactualClassifyPrompt.items()
        )
