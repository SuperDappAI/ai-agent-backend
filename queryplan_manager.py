...
class QueryPlanManager:
    def __init__(self):
        ...
        QA = "Role: Question/Answer. Steps: 1. Understand user's query for concise, accurate answers. 2. Use 'gatherUserInput' for additional user info if needed. 3. Run 'getInformationFromMemory' for relevant historical context. 4. Apply 'webSearch' and semantic URL lookup for more info. 5. Use 'getExternalFunctions' for helpful external code. 6. Draft hypothetical answers. 7. Conduct a final 'webSearch' to refine answer. 8. Respond in specified format with gathered info. 9. For onboarding queries, use 'gatherUserInput' for preferences and relevant links. 10. Use 'getSuperdappDocs' for onboarding details. 11. Fetch onboarding tools with 'getExternalFunctions'. 12. Update user context by responding such that inference will update preference or schedule queries. 13. Respond in specified format with necessary info."
        ...
        Emotion = "Role: Empathy. Steps: 1. Define query and available functions. 2. Use 'gatherUserInput' for user query details. 3. Run 'getInformationFromMemory' for historical context. 4. Apply 'webSearch' for extra query-related info. 5. Assess need to update user preferences via response crafting for inference. 6. Analyze info for an appropriate response considering traits, mood, and goals. 7. Update user preferences for mood changes post-response. 8. Provide a compassionate, supportive response. 9. Update preferences for trait or interest changes post-response. 10. Check user's privacy settings and respect DND times and preferred contact methods. 11. Use 'webSearch' again for extra resources if needed. 12. Finalize user preferences updates post-query. 13. End query planning."
