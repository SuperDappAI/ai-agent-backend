...
class FunctionsManager:
    ...
    def transform(self, user_id, data, category):
        ...
        for item in data:
            ...
            doc = Document(page_content=json.dumps(page_content), metadata=metadata)
            result.append(doc)
        return result
        ...
        if self.inited is False:
            try:
                ...
                with open("./utils/functions.json", "r") as f:
                    ...
                    await self.push_functions(
                        function_input.user_id, function_input.api_key, functions_json
                    )
            self.inited = True
        ...
        try:
            for action_item in function_input.action_items:
                ...
                documents = await self.get_retrieved_nodes(
                    memory, query, action_item.category, function_input.user_id
                )
                if len(documents) > 0:
                    ...
                    await self.rate_limiter.execute(
                        memory.base_retriever.vectorstore.aadd_documents,
                        documents,
                        ids=ids,
                    )
    ...
    async def get_retrieved_nodes(
        self,
        memory: ContextualCompressionRetriever,
        query_str: str,
        category: str,
        user_id: str,
    ):
    ...
    async def push_functions(self, user_id: str, api_key: str, functions):
        ...
        try:
            ...
            await self.rate_limiter.execute(
                memory.base_retriever.vectorstore.aadd_documents, all_docs, ids=ids
            )
            ...
            logging.info(f"FunctionsManager: push_functions took {end - start} seconds")
